  ...
  try:
    ...
    for k in d:
      ...
      try:
        ...
        tests = [jsonpickle.encode,repr,str,lambda v: getattr(v,'__class__.__name__')]
        def check_pickleability(obj): return dill.pickles(obj)
        for test in tests:
          try:
            f"{test.__name__}: {dill.pickles(test(d[k]))}\n\n"
