...
class State:
  ...
  def format_call(self):
    ...
    try:
      fmtd_args = {arg:self.frame.f_locals.get(arg,'MISSING') for arg in hunter_args}
      fmtd_args_str = [f"{c(arg,'vars')}={value}" for arg,value in fmtd_args.items()]
      sub_s = ", ".join(fmtd_args_str)
    except:
      with open('format_call.log','a') as f: f.write(fmtd_args_str)
      raise SystemExit()
    s = (
