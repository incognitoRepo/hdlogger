...
class State:
  ...
  def initialize(self):
    ...
    self.stack = []
    self.pickler = self.pickler_init()
    self._call = None
    self._line = None
    self._return = None
  ...
  @property
  def serialized_arg(self):
    special_cases = [(GeneratorType,pickle_generator), (FrameType,pickle_frame_state)]
    copyreg.pickle()
  ...
  def pickler_init(self):
    ...
    def pickle_frame_state(frame):
      kwds = frame.__dict__
      kwds['version'] = 2
      return unpickle_frame_state, ({'f_lineno':frame.f_lineno},)

    def unpickle_frame_state(kwds):
      version = kwds.pop('version',1)
      Unpickleable = type('Unpickleable',(), kwds)
