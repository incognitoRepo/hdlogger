...
def main2():
  ...
  return_value = hd_tracer.return_value
  _output = out.getvalue().splitlines()
  output = "\n".join(_output)
  from ipdb import set_trace as st; st()
  s1 = f"HDLOGGER\n========\nreturn_value: {return_value!r}\ntrace\n-----{output}"
  args = hd_tracer.return_values
  s2 = [f"{dtype}{value}" for dtype,value in args]
  with open('hdlog.log','w') as f: f.write(s1)
  with open('hdlog.args.log','w') as f: f.write(s2)
