...
def main2():
  ...
  for raw,serialized in zip(return_values,serialized_data):
    deserialized = pickle.loads(serialized)
    assert raw == deserialized, f"{raw=}, {serialized=}, {deserialized=}"
  deserialized = hd_tracer.deserialize(serialized_data)
  output = "\n".join( out.getvalue().splitlines() )
  s1 = f"{return_values=}\n{len(serialized_data)=}, {len(output)=}\n"
  s2 = [f"{obj}" for obj in deserialized]
  with open('hdlog.log','w') as f: f.write(s1)
  with open('hdlog.args.log','w') as f: f.write("\n".join(s2))
