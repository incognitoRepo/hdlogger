...
def main2():
  ...
  with open('return_values.log','w') as f: f.write("\n".join([str(elm) for elm in return_values]))
  serialized_data = [str(elm) if isinstance(elm,bytes) else elm for elm in hd_tracer.serialized_data]
  deserialized = hd_tracer.deserialize(serialized_data)
  output = "\n".join( out.getvalue().splitlines() )
  # from ipdb import set_trace as st; st()
  with open('serialized.log','wb') as f: f.write(b"\n".join([elm for elm in serialized_data]))
  with open('_deserialized.log','w') as f:
    outstr = map(lambda x: str(x) if isinstance(x,bytes) else x,[pickle.loads(elm) for elm in serialized_data])
    f.write("\n".join(outstr))
