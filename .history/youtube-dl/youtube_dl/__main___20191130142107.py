...
def main2():
  ...
  with captured_output() as (out,err):
    try:
      ...
      with open('logs/hdlog.main.log','w') as f: f.write(s)
      raise
    finally:
      return_values = hd_tracer.return_values
      deserialized = hd_tracer.deserialize()
      output = out.getvalue()


      write_file(pformat(return_values),'logs/return_values.log')
      # write_file(pformat(serialized_as_hex),'logs/serialized_as_hex.log')
      # write_file(pformat(deserialized_to_pyobj),'logs/deserialized_to_pyobj.log')
      write_file(pformat(deserialized),'logs/tracer.deserialized_arg.log',mode="a")
      write_file(output,'logs/output.log')
