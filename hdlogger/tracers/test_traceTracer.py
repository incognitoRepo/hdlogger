...
import trace, hdlogger
from prettyprinter import pformat
from hdlogger.tracers import traceTracer as Trace
...
class TestCallers(unittest.TestCase):
  ...
  def test_loop_caller_importing(self):
    ...
    expected = {
      ((os.path.splitext(hdlogger.tracers.DEtrace.__file__)[0] + '.py', 'DEtrace', 'Trace.runfunc'),
        (self.filemod + ('traced_func_importing_caller',))): 1,
...
class TestCoverage(unittest.TestCase):
  ...
  def test_coverage_ignore(self):
    ...
    tracer = Trace(ignoredirs=[sys.base_prefix, sys.base_exec_prefix,
    ...
    if os.path.exists(TESTFN):
      ...
      self.assertEqual(files, ['IPython.core.compilerop.cover'])  # Ignore __import__
...
class TestCoverageCommandLineOutput(unittest.TestCase):
  ...
  def test_cover_files_written_no_highlight(self):
    ...
    # (issue #34171).
    tracedir = os.path.dirname(os.path.abspath(hdlogger.tracers.DEtrace.__file__))
    tracecoverpath = os.path.join(tracedir, 'DEtrace.cover')
